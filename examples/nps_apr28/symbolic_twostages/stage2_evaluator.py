"""
Stage 2 Evaluator: feature engineering + XGBoost.

Evaluates the `engineer_features` function by:
1. Running it on train/val data
2. Training an XGBoost classifier on original + engineered features
3. Reporting ROC AUC, macro F1, and number of engineered features

This mirrors the evaluator pattern from examples/nps/evaluator.py.
"""
...
def evaluate_stage1(program_path):
    ...
    try:
        ...
        val_sample = pd.read_csv(
            os.path.join(DATA_DIR, "val_derived.csv.gz"), nrows=50
        )
